...
class JanelaCodUp:
    ...
    def __init__(self, raiz):
        ...
        if fileName:
            ...
            self.janela.protocol("WM_DELETE_WINDOW",self.FechaJanela)  # Colocando a fun��o de fechar a janela
            self.Forca_Focus()
            
            # Pegando posi��o da janela raiz e da toplevel
            ...
            Label(self.janela, text="Produtos j� Cadastrados ", font=("Helvetica", 10)).grid(row=12, column=2, sticky=W)
    ...
    def CriaTreeView(self, frame, numlinhas, numcolunas, columnspan, rowspan):
        ...
        # a possi��o da barra de rolagem recebe a possi��o inicial do grid + o seu tamanho mais 1
        ...
        # Coloca titulos nas colunas, falta criar fun�ao de ordena��o
        for col in colunas:
    ...
    def Enviar(self):
        ...
        if tkMessageBox.askokcancel("Enviar para o Banco?", "Voc� tem certeza que deseja atualizar estes codigos de barras?"):
            # Acessando o Banco
            self.Conecta()
    ...
    def FechaJanela(self):
        ...
        """
        if tkMessageBox.askokcancel("Exit?", "Voc� tem certeza que deseja fechar?"):
            self.janela.destroy()
            self.janela = None
        else:
    ...
    def Forca_Focus(self, event=None):
        """
        For�a o focus na janela etiquetas
        """
...
class Listas:
    ...
    def __init__(self, janelaraiz):
        ...
        self.janelaetiquetas.title("Impress�o de Lista de Produtos")
        #self.janelaetiquetas.resizable(FALSE,FALSE)
        self.janelaetiquetas.protocol("WM_DELETE_WINDOW",self.FechaJanela)  # Colocando a fun��o de fechar a janela
        ...
        # Descric�o
        Label(self.janelaetiquetas, text="Descri��o").grid(row=2, column=2)
        ...
        Label(frame, text="Selecione os item que aparecer�o na lista",font=("Helvetica", 12)).grid(row=0, column=0, columnspan=4)
        ...
        Checkbutton(
            frame2, text="Itens n�o cadastrados", variable=self.opcoes[-1],
    ...
    def AlteraQTD(self, event):
        ...
        valores = item.values()
        # Verifica se dados n�o est�o vazio
        if valores[2] != '':
    ...
    def filtraProdutos(self, event):
        ...
        if event.widget._name == "jnetdescricao":  # Entry de descricao da janela etiquetas
            ...
            self.jnetcodproduto.delete(0,END)
            # Caso seja seta para cima ou para baixo n�o entra 
            if (event.keycode != 38) and (event.keycode != 40):
                
                
                if texto != "": # Texto vazio n�o faz nada
                    # Apaga toda a treeview para ser inserida uma nova
                    x = self.jnettreeview.get_children() 
                    for item in x: 
                        self.jnettreeview.delete(item)
                    ...
                    if len(texto) <= 3:
                        ...
                        # Criando a expres�o regular para a pesquisa
                        regex = ''
                        # Explodindo texto para fazer a pesquisa por expres�o regular
    ...
    def CriaLista(self):
        ...
        if fileName:
            tipo = fileName.split(".")
            # For�ando para ser um arquivo .csv
            if tipo[-1] != "pdf":
                fileName += ".pdf"
...
class JanelaRaiz:
    """
    Classe com as principais fun��es de gerar a janela raiz do sistema.
    """
    def __init__(self):
        ...
        self.janelaraiz.protocol("WM_DELETE_WINDOW",self.FechaJanela)  # Colocando a fun��o de fechar a janela
    ...
    def IconMenu(self):
        ...
        imgetiquetas = PhotoImage(file="iconImpressaoEtiquetas.gif") # imagem do bot�o gera etiquetas
        ...
        btngeraretiqueta.image = imgetiquetas # salvando a imagem no bot�o para o garbage collection???
        
        imgctlestoque = PhotoImage(file="iconControlEstoque.gif") # imagem do bot�o gera etiquetas
        ...
        btnctlestoque.image = imgctlestoque # salvando a imagem no bot�o para o garbage collection???
        
        
        imgCodBarrasUp = PhotoImage(file="database_up.gif") # imagem do bot�o gera etiquetas
        ...
        btnCodBarrasUp.image = imgCodBarrasUp # salvando a imagem no bot�o para o garbage collection???
        
        #
        imgGeraLista = PhotoImage(file="document-print.gif") # imagem do bot�o gera etiquetas
        btnGeraLista = Button(
            self.janelaraiz,
            compound=TOP,
            width=70,
        ...
        btnGeraLista.image = imgGeraLista # salvando a imagem no bot�o para o garbage collection???
...
class JanelaCTLEstoque:
    ...
    def __init__(self, janelaraiz):
        ...
        self.janelactlestoque.protocol("WM_DELETE_WINDOW",self.FechaJanela)  # Colocando a fun��o de fechar a janela
        #self.janelaetiquetas.focus_set()
        self.Forca_Focus()
        ...
        Label(self.janelactlestoque, text="Produtos j� Cadastrados ", font=("Helvetica", 10)).grid(row=12, column=2, sticky=W)
            ...
            if len(EAN13) == 13:
                ...
                if bar.verifyChecksum(EAN13):
                    ...
                    if produto != None:
                        ...
                        # Codigo de Barras lido ainda n�o foi cadastrado
                        self.alerta.bepp()
                        if tkMessageBox.askokcancel("Aten��o", "Produto n�o cadastrado, deseja cadastra-lo?"):
...
class JanelaAlteraProduto:
    ...
    def __init__(self, janelaraiz, codBarras):
        ...
        # Descric�o
        Label(self.janela, text="Descri��o").grid(row=0, column=2)
...
class JanelaProduto:
    """
    Classe responsavel por gerar as janela de exibi��o e altera��o de produto
    """
    ...
    def __init__(self, magicnumber=15):
        ...
        edi��o. Por padr�o todos os campos est�o disponiveis, magicnumber=15.
        ...
        self.cancela = False  # Usado quando a foi fechado sem salvar ou o bot�o cancelar foi apertado
        #Pega valores do item
        ...
        if valores[2] != '':
            ...
            self.alteraJn.protocol("WM_DELETE_WINDOW",self.FechaJanela)  # Colocando a fun��o de fechar a janela
            ...
            # For�ando o focus na janela, e bloqueando nela
            ...
            self.alteraJn.resizable(FALSE,FALSE)  # Bloqueando altera��o de tamanho
            self.CalculaMagicNumber()
            
    def CalculaMagicNumber(self):
        """
        Leva em conta o magic number passado na inicializa��o para tornanr os campos editaveis ou n�o.
        Utiliza altera��o de focus para fazer o entry n�o ser editavel
        """
        #self.magicnumber = 14
        ...
        if (self.magicnumber <= 15) and (self.magicnumber >= 1):
            ...
            if (int(resto)/1 == 1):
             ...
             """ 
            # Retira focus caso n�o seja editavel
    ...
    def get(self):
        """
        Retorna os valores alterados na janela de altera��o.
        """
...
class JanelaEtiquetas:
    ...
    def __init__(self, janelaraiz):
        ...
        # Criando Janela TopLevel
        self.janelaetiquetas = Toplevel()
        self.janelaetiquetas.title("Impress�o de Etiquetas")
        self.janelaetiquetas.resizable(FALSE,FALSE)
        self.janelaetiquetas.protocol("WM_DELETE_WINDOW",self.FechaJanela)  # Colocando a fun��o de fechar a janela
        #self.janelaetiquetas.focus_set()
        self.Forca_Focus()
        ...
        # Descric�o
        Label(self.janelaetiquetas, text="Descri��o").grid(row=0, column=2)
        self.jnetdescricao = Entry(self.janelaetiquetas, name="jnetdescricao")
        self.jnetdescricao["width"] = 50
        ...
        # Descric�o
        Label(self.janelaetiquetas, text="Descri��o").grid(row=13, column=2)
    ...
    def GerarEtiquetas(self):
        """
        Chama a fun��o para gerar o pdf com as etiquetas passadas.
        """
        ...
        if self.produtosselecionados.ContaTotalProdutos() > 0:
            ...
            if barCodeVali == True:
                ...
                while 1:  
                    ...
                    if fileName: #  Caso o bot�o cancelar n�o tenha sido selecionado, n�o entra
                        tipo = fileName.split(".")
                        
                        # Verifica se nome n�o esta em branco
                        if tipo[0] == "":
                            continue
                        # For�ando para ser um arquivo .csv
                        if tipo[-1] != "pdf":
                        ...
                        try:
                            ...
                            tkMessageBox.showinfo(title="Aten��o", message="Arquivo em uso por outro programa ou sem permiss�o de altera��o.\nEscolha Outro!")
                            #print "Arquivo aberto"
                        else:
                            ...
                            tkMessageBox.showinfo(title="Parabens", message="Gera��o de Etiquetas realizada com sucesso!")
                            apagaTreeView(self.jnettreeview2)
                            self.produtosselecionados = listaProdutos()
            ...
            tkMessageBox.showinfo(title="Aten��o", message="Selecione Algum Produto!")
